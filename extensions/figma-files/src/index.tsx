...
function Command({
	launchContext,
}: Readonly<LaunchProps<{ launchContext: { query: string } }>>) {
	const {
		data,
		isLoading,
		error,
	...
	const isLoadingBlock =
		isLoading || isLoadingVisitedFiles || isLoadingStarredFiles;
	...
	function handleDropdownChange(value: string) {
		if (data && !isLoading) {
			if (value === "All") {
				...
				setFilteredFiles(
					data.filter((team) => team.name === value.split("=")[1]),
				);
				setIsFiltered(true);
			} else {
				setFilteredFiles([
					{
						...
						files: data
							...
							.files.filter(
								(project) => project.name === value.split("&$%")[1],
							),
					} as TeamFiles,
				]);
				setIsFiltered(true);
	...
	const teamID: string[] = TEAM_ID.split(",").map((team: string) =>
		team.toString().trim(),
	);
	const filterDropdown = () => (
		...
		>
			<Grid.Dropdown.Item
				key="all"
				title={teamID.length > 1 ? "All teams" : "All projects"}
				value="All"
			/>
			{teamID.length > 1 &&
				data?.map((team) => (
			...
			{filteredFiles?.map((team) =>
				team.files.map((project) =>
					project.files?.length != 0 ? (
						...
						>
							<Grid.Item
								key={project.name + "-file-empty"}
								content="emptyProject.svg"
								title="Empty project"
							/>
						</Grid.Section>
					),
